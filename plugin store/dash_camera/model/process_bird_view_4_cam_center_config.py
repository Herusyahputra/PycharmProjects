...
def merge_image_4_camera_center_config(image_crop, properties_image, shift_x, shift_y, gradient_mode):
    ...
    merge_image_canvas = np.zeros([int(height), width, 3], dtype=np.uint8)
    ...
    if overlap_front_right is not None:
        ...
        try:
            bird_view[pos_y_2 + pos_fl_y:pos_y_2 + pos_fl_y + blending_front_right.shape[0],
            pos_x_2: pos_x_2 + blending_front_right.shape[1]] = blending_front_right
        except:
            pass

    overlap_left_rear, overlap_right_rear, left_rear_ov, rear_left_ov, right_rear_ov, rear_right_ov = \
        rear_image_overlap(image_crop, shift_x, shift_y, properties_image)
